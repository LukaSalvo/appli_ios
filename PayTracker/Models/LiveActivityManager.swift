...
enum LiveActivityManager {
    #if canImport(ActivityKit)
    static func start(startDate: Date, earned: Double, ratePerHour: Double) {
        ...
        guard Activity<PayActivityAttributes>.activities.isEmpty else {
            update(startDate: startDate, earned: earned, ratePerHour: ratePerHour)
            return
        }
        let attributes = PayActivityAttributes(title: "Paie en cours")
        let state = PayActivityAttributes.ContentState(
            startDate: startDate, earned: earned, ratePerHour: ratePerHour
        )
        do {
    ...
    static func update(startDate: Date, earned: Double, ratePerHour: Double) {
        let state = PayActivityAttributes.ContentState(
            startDate: startDate, earned: earned, ratePerHour: ratePerHour
        )
        Task {
